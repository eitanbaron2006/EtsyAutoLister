...
function TipFillerCard({ offset = 0, savedTips, onToggleSave }: TipSaveProps & { offset?: number }) {
...
function TipPanel({ savedTips, onToggleSave }: TipSaveProps) {
...
export default function Home() {
  ...
  // Personal account data (synced with the Firestore user profile)
  ...
  // Save / unsave a studio tip to the user's account page (synced to Firestore)
                  ...
                  {viewerMockups.length > 0 ? (
                    ...
                    >
                      ...
                      {Array.from({ length: viewerEmptyCells }).map((_, fillerIndex) => (
                        <TipFillerCard key={`tip-filler-${fillerIndex}`} offset={fillerIndex * 3} savedTips={savedTips} onToggleSave={handleToggleSavedTip} />
                      ))}
                    </div>
                    {/* A single photo row frees the lower half — tips zone */}
                    {viewerRows === 1 && <TipPanel savedTips={savedTips} onToggleSave={handleToggleSavedTip} />}
                    </>
              ...
              return (
                ...
                >
                  ...
                  {Array.from({ length: inspectorEmptyCells }).map((_, fillerIndex) => (
                    <TipFillerCard key={`tip-filler-${fillerIndex}`} offset={fillerIndex * 3 + 1} savedTips={savedTips} onToggleSave={handleToggleSavedTip} />
                  ))}
                </div>
                {/* A single photo row frees the lower half — tips zone */}
                {inspectorRows === 1 && <TipPanel savedTips={savedTips} onToggleSave={handleToggleSavedTip} />}
                </>
              );
            })()}
          </div>
